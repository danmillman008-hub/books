...
pgrep -f "llm_bridge/server.py" >/dev/null || nohup DeepTutor/.venv/bin/python llm_bridge/server.py > study/logs/bridge.log 2>&1 &
sleep 2
pgrep -f "deeptutor serve" >/dev/null || (cd DeepTutor && nohup .venv/bin/deeptutor serve --host 127.0.0.1 --port 8001 > ../study/logs/api.log 2>&1 &)
